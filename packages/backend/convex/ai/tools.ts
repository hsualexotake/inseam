...
export const searchNotes = createTool({
  ...
  handler: async (ctx: ToolCtx, { query, limit = 5 }) => {
...
export const createNote = createTool({
  ...
  handler: async (ctx: ToolCtx, { title, content, generateSummary = false }) => {
...
export const analyzeNotes = createTool({
  ...
  handler: async (ctx: ToolCtx, { analysisType, limit = 10 }) => {
    const notes = await ctx.runQuery(api.notes.getNotes, {});
        ...
        return {
          analysis: {
            ...
            averageLength: Math.round(
              recentNotes.reduce((acc, note) => acc + note.content.length, 0) / recentNotes.length
            ),
            withSummaries: recentNotes.filter(n => n.summary).length,
          },
